-- Function: xt.parsemetasql(text, text)

-- DROP FUNCTION xt.parsemetasql(text, text);
...
$BODY$
  ...
  /**
   * This is the PEG.js generated MetaSQL parser code from:
   * https://github.com/bendiy/mql-parser/blob/master/metasqlqueryparser-basic.peg
   * 
   * WARNING!!! IMPORTANT!!! README!!!
   *
   * The code below is GENERATED. Other than a few lines at the bottom of this
   * function that call the parser, none of this was written by a human.
   *
   * DO NOT EDIT this generated code! If you find a bug or need to extend it
   * with a new feature, you MUST follow this process:
   *
   * 1. Modify the MetaSQL Grammer definition linked above.
   * 2. Add tests to this query to cover the bug or new feature:
   *    - https://github.com/xtuple/openrpt/blob/master/MetaSQL/test.mql
   * 3. Run the test in the old C++ MetaSQL parser and make sure it passes there.
   * 4. Use the PEG.js online tool to test the new grammer with the test query:
   *    - http://pegjs.org/online
   * 5. If all tests pass, use the online tool (or command line) to generate NEW
   *    parser code and replace the generated parser code below with it.
   * 6. Run the MetaSQL test query through this function and ensure it passes.
   *
   * TODO:
   * - In it's current state, this parser is designed to mimic the old C++
   *   MetaSQL parser worts and all. This is to ensure backwards compatibility
   *   for any existing MetaSQL. Add a legacy options flag for any code that
   *   will break that compatibility.
   * - This parser could be extended to support a non-legacy parser. It will be
   *   called by setting a legacy flag to false in the options parameter. 
   * - The non-legacy parser should support working continue() and break()
   *   functions in foreach loops.
   * - The non-legacy parser could output plv8 paramerterized queries:
   *    (SELECT %1$I FROM %2$I.%3$I WHERE %4$I = $1)
   *   That paramerterized query can then be passed through PostgreSQL's
   *   format() function to generate SQLi safe query strings and then executed
   *   through plv8.execute(sql-here, [params-array-here]).
   */
  var parser = (function() {
    /*
  ...
  /**
   * End of PEG.js generated MetaSQL parser code.
   */ 
   ...
   */
  try {
    sql += parser.parse(metasql, JSON.parse(options));
  } catch (err) {
    // Mimic mobilized XT.error() style error message.
    var message = {
      code: 500, 
      message: 'Cannot parse MetaSQL.'
    };
...
ALTER FUNCTION xt.parsemetasql(text, text)
  OWNER TO admin;
